...
- [x] AI powered suggestions (levels, setups, entries, exits, etc) — core loop built, see [docs/trade-analysis-plan.md](docs/trade-analysis-plan.md)
  ...
  - [x] Real desktop notifications (Web Notifications API) for triggered alerts, opt-in via the bell icon — `data/alerts.json` is the only thing any skill needs to write to; the notification system is generic on top of it
  - [x] Condition-triggered alerts (`vite-plugins/marketData/alertsEngine.ts`) — Claude writes a concrete, checkable condition (price/level cross, EMA/MACD crossover, RSI/ATR threshold) instead of a scan-time fact; a background engine evaluates pending conditions on the same cadence the cache refreshes on (live ~10s on-screen, 5-min background) and flips them to triggered — that's what the desktop notification actually fires on now, not alert creation. Alerts carry a plain-language headline/rationale/actionGuidance plus the raw condition behind an expandable "technical conditions" view; superseded/expired/acknowledged alerts get their open OS notification actively closed, not just suppressed from re-popping
  ...
  - [x] `analysis.md` (was `analysis.json`) — replaced the collapsed per-timeframe digest (a stored `swingHigh`, `emaTrend`, etc.) with the full annotated candle history per timeframe as markdown tables: every row precomputed with indicators + candlestick patterns, nothing collapsed. Node still owns 100% of the deterministic math; trend structure, momentum crossovers, level identification, and cross-timeframe synthesis are explicitly `find-trades`' job now, reading the tables the way it'd read a chart — that's the actual point of exposing full history instead of a summary
  - [x] Period-partitioned cache (superseded the single combined `analysis.md` above) — the persisted `.json` cache now carries the indicators/patterns itself (computed once, not re-derived on every read) and is split by period per timeframe (day files for `1m`/`5m`/`15m`, ISO-week for `1h`, month for `1d`, single file for `1w`), each mirrored by a `.md` file `find-trades` actually reads, plus a small cross-timeframe `latest.md` for quick orientation. Lets a read pull exactly the history a setup needs (today's `1m` for an ORB read, months of `1d` for a structure read) instead of one giant per-timeframe file
